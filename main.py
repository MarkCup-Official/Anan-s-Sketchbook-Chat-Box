...
from config import DELAY, FONT_FILE,BASEIMAGE_MAPPING,BASEIMAGE_FILE, AUTO_SEND_IMAGE, AUTO_PASTE_IMAGE, BLOCK_HOTKEY, HOTKEY, SEND_HOTKEY,PASTE_HOTKEY,CUT_HOTKEY,SELECT_ALL_HOTKEY,TEXT_BOX_TOPLEFT,IMAGE_BOX_BOTTOMRIGHT,BASE_OVERLAY_FILE,USE_BASE_OVERLAY, ALLOWED_PROCESSES, IMAGE_BOX_CONFIGS

from text_fit_draw import draw_text_auto
from image_fit_paste import paste_image_auto
current_image_file = BASEIMAGE_FILE

def get_image_box_config(image_path):
    #根据图片路径获取对应的文本框配置

    # 获取图片文件名（不包含路径）
    image_filename = os.path.basename(image_path)
    
    print(f"查找图片配置: {image_path} -> 文件名: {image_filename}")
    
    # 如果在配置中找到了对应的设置，使用该设置
    if image_filename in IMAGE_BOX_CONFIGS:
        config = IMAGE_BOX_CONFIGS[image_filename]
        print(f"找到特定配置: {config}")
        return config
    
    # 否则使用默认配置
    print(f"使用默认配置: {IMAGE_BOX_CONFIGS['default']}")
    return IMAGE_BOX_CONFIGS["default"]
...
def Start():
    ...
    if image is not None:
        ...
        # 获取当前图片对应的文本框配置
        text_box_topleft, image_box_bottomright = get_image_box_config(current_image_file)
        print(f"使用图片: {current_image_file}, 文本框位置: {text_box_topleft} -> {image_box_bottomright}")
